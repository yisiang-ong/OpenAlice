...
| Edge measurement | `backtestSignal` — every historical firing of a signal spec replayed through an ATR/pct stop + R-multiple target bracket (intrabar, stop wins ties, non-overlapping); win rate, avg R, expectancy, small-sample flag. `simulate` gained the same `bracket` exit rule for single entries | `src/domain/analysis/signal/backtest.ts`, `src/domain/analysis/simulate.ts`, CLI `alice analysis backtest` |
...
teaching messages and never block `closePosition` — you can always get out.
